...
with open('schoolOutputData.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    writer.writerow(['id', 'name', 'capacity', 'address', 'city', 'state', 'zipCode', 'timeOfDay', 'startTime'])
    for school in schoolObjects:
        writer.writerow([school.id, school.name, school.capacity, school.address, school.city, school.state, school.zipCode, school.timeOfDay, school.startTime])